...
module std::features {
    ...
    /// Whether the new `depay_stdlib::type_info::chain_id()` native for fetching the chain ID is enabled.
    /// This is needed because of the introduction of a new native function.
    /// Lifetime: transient
    const DEPAY_STD_CHAIN_ID_NATIVES: u64 = 4;

    public fun get_depay_stdlib_chain_id_feature(): u64 { DEPAY_STD_CHAIN_ID_NATIVES }

    public fun depay_stdlib_chain_id_enabled(): bool acquires Features {
        is_enabled(DEPAY_STD_CHAIN_ID_NATIVES)
    }
    ...
    /// Whether the new `depay_stdlib::multi_ed25519::public_key_validate_internal_v2()` native is enabled.
    /// This is needed because of the introduction of a new native function.
    /// Lifetime: transient
    ...
    /// Lifetime: transient
    const DEPAY_UNIQUE_IDENTIFIERS: u64 = 23;
    ...
    /// Deprecated by `depay_framework::randomness_config::RandomnessConfig`.
    ...
    /// Deprecated by `depay_framework::jwk_consensus_config::JWKConsensusConfig`.
    ...
    const PRIMARY_DPT_FUNGIBLE_STORE_AT_USER_ADDRESS: u64 = 61;

    #[deprecated]
    public fun get_primary_dpt_fungible_store_at_user_address_feature(
    ): u64 {
        abort error::invalid_argument(EINVALID_FEATURE)
    }

    #[deprecated]
    public fun primary_dpt_fungible_store_at_user_address_enabled(): bool acquires Features {
        is_enabled(PRIMARY_DPT_FUNGIBLE_STORE_AT_USER_ADDRESS)
    }
    ...
    /// Lifetime: transient
    const NEW_ACCOUNTS_DEFAULT_TO_FA_DPT_STORE: u64 = 64;

    public fun get_new_accounts_default_to_fa_dpt_store_feature(): u64 { NEW_ACCOUNTS_DEFAULT_TO_FA_DPT_STORE }

    public fun new_accounts_default_to_fa_dpt_store_enabled(): bool acquires Features {
        is_enabled(NEW_ACCOUNTS_DEFAULT_TO_FA_DPT_STORE)
    }

    /// Lifetime: transient
    const OPERATIONS_DEFAULT_TO_FA_DPT_STORE: u64 = 65;

    public fun get_operations_default_to_fa_dpt_store_feature(): u64 { OPERATIONS_DEFAULT_TO_FA_DPT_STORE }

    public fun operations_default_to_fa_dpt_store_enabled(): bool acquires Features {
        is_enabled(OPERATIONS_DEFAULT_TO_FA_DPT_STORE)
    }

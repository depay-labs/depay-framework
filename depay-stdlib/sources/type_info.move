module depay_std::type_info {
    ...
    /// Returns the current chain ID, mirroring what `depay_framework::chain_id::get()` would return, except in `#[test]`
    /// functions, where this will always return `4u8` as the chain ID, whereas `depay_framework::chain_id::get()` will
    /// return whichever ID was passed to `depay_framework::chain_id::initialize_for_test()`.
    public fun chain_id(): u8 {
        if (!features::depay_stdlib_chain_id_enabled()) {
    ...
    fun test_type_of() {
        ...
        assert!(account_address(&type_info) == @depay_std, 0);
    ...
    fun test_chain_id(fx: signer) {
        ...
        features::change_feature_flags_for_testing(&fx, vector[features::get_depay_stdlib_chain_id_feature()], vector[]);
    ...
    fun verify_type_of() {
        ...
        spec {
            assert account_address == @depay_std;

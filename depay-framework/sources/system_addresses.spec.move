spec depay_framework::system_addresses {
    /// <high-level-req>
    /// No.: 1
    ...
    ///
    /// No.: 2
    /// Requirement: Asserting that a provided address corresponds to the DePay Framework Resources address should always
    /// yield a true result when matched.
    /// Criticality: High
    /// Implementation: The assert_depay_framework function ensures that the provided signer belongs to the
    /// @depay_framework account.
    /// Enforcement: Formally verified via [high-level-req-2](AbortsIfNotDePayFramework).
    ///
    /// No.: 3
    ...
    spec assert_depay_framework(account: &signer) {
        pragma opaque;
    ...
    /// Specifies that a function aborts if the account does not have the depay_ framework address.
    spec schema AbortsIfNotDePayFramework {
        account: signer;
        /// [high-level-req-2]
        aborts_if signer::address_of(account) != @depay_framework with error::PERMISSION_DENIED;
    }

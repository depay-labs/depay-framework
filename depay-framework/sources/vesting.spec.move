spec depay_framework::vesting {
    ...
    /// period.
    /// Enforcement: Audited the check for the end of vesting cliff: [https://github.com/depay_-labs/depay_-core/blob/main/depay_-move/framework/depay_-framework/sources/vesting.move#L566](vest) module.
    ///
    /// No.: 5
    ...
    /// address, that it is registered for dpt, that the list of shareholders is non-empty, and that the amount of
    ...
    spec create_vesting_contract {
        ...
        pragma verify = false;
        /// [high-level-req-10]
        aborts_if withdrawal_address == @depay_framework || withdrawal_address == @vm_reserved;
        aborts_if !exists<account::Account>(withdrawal_address);
        aborts_if !exists<coin::CoinStore<DePayCoin>>(withdrawal_address);
    ...
    spec set_beneficiary(
        ...
        aborts_if !coin::spec_is_account_registered<DePayCoin>(new_beneficiary);
        include VerifyAdminAbortsIf;
    ...
    spec create_vesting_contract_account(
        ...
        aborts_if !exists<coin::CoinStore<DePayCoin>>(resource_addr) && !depay_std::type_info::spec_is_struct<DePayCoin>();
        aborts_if !exists<coin::CoinStore<DePayCoin>>(resource_addr) && ea && acc.guid_creation_num + 2 > MAX_U64;
        aborts_if !exists<coin::CoinStore<DePayCoin>>(resource_addr) && ea && acc.guid_creation_num + 2 >= account::MAX_GUID_CREATION_NUM;
        ensures exists<account::Account>(resource_addr) && post_acc.authentication_key == account::ZERO_AUTH_KEY &&
                exists<coin::CoinStore<DePayCoin>>(resource_addr);
    ...
    spec withdraw_stake(vesting_contract: &VestingContract, contract_address: address): Coin<DePayCoin> {
        // TODO: Calls `staking_contract::distribute` which is not verified.
        pragma verify = false;
    ...
    spec schema DistributeInternalAbortsIf {
        ...
        aborts_if !exists<stake::ValidatorSet>(@depay_framework);
        let validator_set = global<stake::ValidatorSet>(@depay_framework);

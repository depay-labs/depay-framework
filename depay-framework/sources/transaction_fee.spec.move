spec depay_framework::transaction_fee {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: Given the blockchain is in an operating state, it guarantees that the DePay framework signer may burn
    /// DePay coins.
    /// Criticality: Critical
    /// Implementation: The DePayCoinCapabilities structure is defined in this module and it stores burn capability to
    /// burn the gas fees.
    ...
    /// Criticality: Critical
    /// Implementation: The initialize_fee_collection_and_distribution function ensures only the DePay framework address
    /// calls it.
    ...
    ///
    /// No.: 6
    /// Requirement: The presence of the resource, indicating collected fees per block under the DePay framework account,
    ...
    /// process_collected_fees all ensure that the CollectedFeesPerBlock resource exists under depay_framework by
    ...
    spec module {
        ...
        // property 1: Given the blockchain is in an operating state, it guarantees that the DePay framework signer may burn DePay coins.
        /// [high-level-req-1]
        invariant [suspendable] chain_status::is_operating() ==> exists<DePayCoinCapabilities>(@depay_framework) || exists<DePayFABurnCapabilities>(@depay_framework);
    }
    ...
    spec initialize_fee_collection_and_distribution(_depay_framework: &signer, _burn_percentage: u8) {
    }

    /// `DePayCoinCapabilities` should be exists.
    spec burn_fee(account: address, fee: u64) {
        use depay_std::type_info;
        use depay_framework::optional_aggregator;
        use depay_framework::coin;
        use depay_framework::coin::{CoinInfo, CoinStore};
        // TODO(fa_migration)
        pragma verify = false;

        aborts_if !exists<DePayCoinCapabilities>(@depay_framework);

        // This function essentially calls `coin::burn_coin`, monophormized for `DePayCoin`.
        let account_addr = account;
        let amount = fee;

        let depay_addr = type_info::type_of<DePayCoin>().account_address;
        let coin_store = global<CoinStore<DePayCoin>>(account_addr);
        let post post_coin_store = global<CoinStore<DePayCoin>>(account_addr);

        // modifies global<CoinStore<DePayCoin>>(account_addr);

        aborts_if amount != 0 && !(exists<CoinInfo<DePayCoin>>(depay_addr)
            && exists<CoinStore<DePayCoin>>(account_addr));
        aborts_if coin_store.coin.value < amount;

        let maybe_supply = global<CoinInfo<DePayCoin>>(depay_addr).supply;
        let supply_aggr = option::spec_borrow(maybe_supply);
        let value = optional_aggregator::optional_aggregator_value(supply_aggr);

        let post post_maybe_supply = global<CoinInfo<DePayCoin>>(depay_addr).supply;
        ...
        ensures coin::supply<DePayCoin> == old(coin::supply<DePayCoin>) - amount;
    }
    ...
    spec mint_and_refund(account: address, refund: u64) {
        ...
        aborts_if (refund != 0) && !exists<CoinInfo<DePayCoin>>(depay_addr);
        include coin::CoinAddAbortsIf<DePayCoin> { amount: refund };

        aborts_if !exists<CoinStore<DePayCoin>>(account);
        // modifies global<CoinStore<DePayCoin>>(account);

        aborts_if !exists<DePayCoinMintCapability>(@depay_framework);

        let supply = coin::supply<DePayCoin>;
        let post post_supply = coin::supply<DePayCoin>;
        aborts_if [abstract] supply + refund > MAX_U128;
        ensures post_supply == supply + refund;
    }

    /// Ensure caller is admin.
    /// Aborts if `DePayCoinCapabilities` already exists.
    spec store_depay_coin_burn_cap(depay_framework: &signer, burn_cap: BurnCapability<DePayCoin>) {
        use std::signer;
        ...
        aborts_if exists<DePayFABurnCapabilities>(addr);
        aborts_if exists<DePayCoinCapabilities>(addr);

        ensures exists<DePayFABurnCapabilities>(addr) || exists<DePayCoinCapabilities>(addr);
    }

    /// Ensure caller is admin.
    /// Aborts if `DePayCoinMintCapability` already exists.
    spec store_depay_coin_mint_cap(depay_framework: &signer, mint_cap: MintCapability<DePayCoin>) {
        use std::signer;
        let addr = signer::address_of(depay_framework);
        aborts_if !system_addresses::is_depay_framework_address(addr);
        aborts_if exists<DePayCoinMintCapability>(addr);
        ensures exists<DePayCoinMintCapability>(addr);
    }

module depay_framework::depay_account {
    use depay_framework::account::{Self, new_event_handle};
    ...
    use depay_framework::event::{EventHandle, emit_event, emit};
    use depay_framework::fungible_asset::{Self, Metadata, BurnRef, FungibleAsset};
    ...
    use std::signer;
    use std::vector;
    use depay_framework::object::Object;

    friend depay_framework::genesis;
    friend depay_framework::resource_account;
    ...
    const EACCOUNT_NOT_REGISTERED_FOR_DPT: u64 = 2;
    ...
    public entry fun transfer(source: &signer, to: address, amount: u64) {
        ...
        if (features::operations_default_to_fa_dpt_store_enabled()) {
            ...
            // Resource accounts can be created without registering them to receive DPT.
            // This conveniently does the registration if necessary.
            if (!coin::is_account_registered<DePayCoin>(to)) {
                coin::register<DePayCoin>(&create_signer(to));
            };
            coin::transfer<DePayCoin>(source, to, amount)
        }
    }
    ...
    public fun deposit_coins<CoinType>(to: address, coins: Coin<CoinType>) acquires DirectTransferConfig {
        if (!account::exists_at(to)) {
            create_account(to);
            spec {
                assert coin::spec_is_account_registered<DePayCoin>(to);
                assume depay_std::type_info::type_of<CoinType>() == depay_std::type_info::type_of<DePayCoin>() ==>
    ...
    public fun assert_account_is_registered_for_dpt(addr: address) {
        assert_account_exists(addr);
        assert!(coin::is_account_registered<DePayCoin>(addr), error::not_found(EACCOUNT_NOT_REGISTERED_FOR_DPT));
    }
    ...
    public(friend) fun register_dpt(account_signer: &signer) {
        if (features::new_accounts_default_to_fa_dpt_store_enabled()) {
    ...
    public(friend) entry fun fungible_transfer_only(
        ...
        // as DPT cannot be frozen or have dispatch, and PFS cannot be transfered
    ...
    inline fun ensure_primary_fungible_store_exists(owner: address): address {
        ...
        if (fungible_asset::store_exists(store_addr)) {
            ...
            object::object_address(&primary_fungible_store::create_primary_store(owner, object::address_to_object<Metadata>(@depay_fungible_asset)))
        }
    }

    /// Address of DPT Primary Fungible Store
    inline fun primary_fungible_store_address(account: address): address {
        object::create_user_derived_object_address(account, @depay_fungible_asset)
    }
    ...
    public fun test_transfer(alice: &signer, core: &signer) {
        ...
        let (burn_cap, mint_cap) = depay_framework::depay_coin::initialize_for_test(core);
        create_account(signer::address_of(alice));
        coin::deposit(signer::address_of(alice), coin::mint(10000, &mint_cap));
        transfer(alice, bob, 500);
        assert!(coin::balance<DePayCoin>(bob) == 500, 0);
        transfer(alice, carol, 500);
        assert!(coin::balance<DePayCoin>(carol) == 500, 1);
        transfer(alice, carol, 1500);
        assert!(coin::balance<DePayCoin>(carol) == 2000, 2);
    ...
    public fun test_transfer_to_resource_account(alice: &signer, core: &signer) {
        ...
        let (burn_cap, mint_cap) = depay_framework::depay_coin::initialize_for_test(core);
        assert!(!coin::is_account_registered<DePayCoin>(resource_acc_addr), 0);

        create_account(signer::address_of(alice));
        coin::deposit(signer::address_of(alice), coin::mint(10000, &mint_cap));
        transfer(alice, resource_acc_addr, 500);
        assert!(coin::balance<DePayCoin>(resource_acc_addr) == 500, 1);
    ...
    public fun test_batch_transfer(from: &signer, core: &signer, recipient_1: &signer, recipient_2: &signer) {
        ...
        assert!(coin::balance<DePayCoin>(recipient_1_addr) == 100, 0);
        assert!(coin::balance<DePayCoin>(recipient_2_addr) == 500, 1);
        coin::destroy_burn_cap(burn_cap);
        coin::destroy_mint_cap(mint_cap);
    }
    ...
    fun test_primary_fungible_store_address(
        ...
        assert!(primary_fungible_store_address(user_addr) == primary_fungible_store::primary_store_address(user_addr, dpt_metadata), 1);

        ensure_primary_fungible_store_exists(user_addr);
        assert!(primary_fungible_store::primary_store_exists(user_addr, dpt_metadata), 2);
    }
}

...
module depay_framework::reconfiguration {
    use std::error;
    use std::features;
    use std::signer;
    ...
    friend depay_framework::consensus_config;
    friend depay_framework::execution_config;
    friend depay_framework::gas_schedule;
    friend depay_framework::genesis;
    friend depay_framework::version;
    ...
    /// Publishes `Configuration` resource. Can only be invoked by depay_ framework account, and only a single time in Genesis.
    public(friend) fun initialize(depay_framework: &signer) {
        ...
        assert!(account::get_guid_next_creation_num(signer::address_of(depay_framework)) == 2, error::invalid_state(EINVALID_GUID_FOR_EVENT));
        move_to<Configuration>(
            depay_framework,
            Configuration {
                epoch: 0,
                last_reconfiguration_time: 0,
                events: account::new_event_handle<NewEpochEvent>(depay_framework),
            }
        );
    }
    ...
    fun disable_reconfiguration(depay_framework: &signer) {
        system_addresses::assert_depay_framework(depay_framework);
        assert!(reconfiguration_enabled(), error::invalid_state(ECONFIGURATION));
        move_to(depay_framework, DisableReconfiguration {})
    }

    /// Private function to resume reconfiguration.
    /// This function should only be used for offline WriteSet generation purpose and should never be invoked on chain.
    fun enable_reconfiguration(depay_framework: &signer) acquires DisableReconfiguration {
        system_addresses::assert_depay_framework(depay_framework);

        assert!(!reconfiguration_enabled(), error::invalid_state(ECONFIGURATION));
        DisableReconfiguration {} = move_from<DisableReconfiguration>(signer::address_of(depay_framework));
    }

    fun reconfiguration_enabled(): bool {
        !exists<DisableReconfiguration>(@depay_framework)
    }
    ...
    public(friend) fun reconfigure() acquires Configuration {
        ...
        let config_ref = borrow_global_mut<Configuration>(@depay_framework);
    ...
    public fun last_reconfiguration_time(): u64 acquires Configuration {
        borrow_global<Configuration>(@depay_framework).last_reconfiguration_time
    }

    public fun current_epoch(): u64 acquires Configuration {
        borrow_global<Configuration>(@depay_framework).epoch
    }

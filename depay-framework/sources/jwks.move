...
module depay_framework::jwks {
    ...
    use std::vector;
    use depay_std::comparator::{compare_u8_vector, is_greater_than, is_equal};
    use depay_std::copyable_any;
    use depay_std::copyable_any::Any;
    use depay_framework::chain_status;
    use depay_framework::config_buffer;
    use depay_framework::event::emit;
    ...
    const EINSTALL_FEDERATED_JWKS_AT_DEPAY_FRAMEWORK: u64 = 7;
    ...
    public fun patch_federated_jwks(jwk_owner: &signer, patches: vector<Patch>) acquires FederatedJWKs {
        // Prevents accidental calls in 0x1::jwks that install federated JWKs at the DePay framework address.
        assert!(!system_addresses::is_depay_framework_address(signer::address_of(jwk_owner)),
            error::invalid_argument(EINSTALL_FEDERATED_JWKS_AT_DEPAY_FRAMEWORK)
        );
    ...
    /// ```
    ///
    /// See AIP-96 for more details about federated keyless - https://github.com/depay_-foundation/AIPs/blob/main/aips/aip-96.md
    ///
    ...
    public fun try_get_patched_jwk(issuer: vector<u8>, jwk_id: vector<u8>): Option<JWK> acquires PatchedJWKs {
        let jwks = &borrow_global<PatchedJWKs>(@depay_framework).jwks;
    ...
    public fun upsert_oidc_provider(fx: &signer, name: vector<u8>, config_url: vector<u8>): Option<vector<u8>> acquires SupportedOIDCProviders {
        system_addresses::assert_depay_framework(fx);
        chain_status::assert_genesis();

        let provider_set = borrow_global_mut<SupportedOIDCProviders>(@depay_framework);
    ...
    public fun upsert_oidc_provider_for_next_epoch(fx: &signer, name: vector<u8>, config_url: vector<u8>): Option<vector<u8>> acquires SupportedOIDCProviders {
        ...
        let provider_set = if (config_buffer::does_exist<SupportedOIDCProviders>()) {
            ...
            *borrow_global<SupportedOIDCProviders>(@depay_framework)
        };
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires SupportedOIDCProviders {
        ...
        if (config_buffer::does_exist<SupportedOIDCProviders>()) {
            ...
            if (exists<SupportedOIDCProviders>(@depay_framework)) {
                *borrow_global_mut<SupportedOIDCProviders>(@depay_framework) = new_config;
            } else {
    ...
    public fun set_patches(fx: &signer, patches: vector<Patch>) acquires Patches, PatchedJWKs, ObservedJWKs {
        system_addresses::assert_depay_framework(fx);
        borrow_global_mut<Patches>(@depay_framework).patches = patches;
    ...
    public fun upsert_into_observed_jwks(fx: &signer, provider_jwks_vec: vector<ProviderJWKs>) acquires ObservedJWKs, PatchedJWKs, Patches {
        system_addresses::assert_depay_framework(fx);
        let observed_jwks = borrow_global_mut<ObservedJWKs>(@depay_framework);
    ...
    fun regenerate_patched_jwks() acquires PatchedJWKs, Patches, ObservedJWKs {
        let jwks = borrow_global<ObservedJWKs>(@depay_framework).jwks;
        let patches = borrow_global<Patches>(@depay_framework);
        vector::for_each_ref(&patches.patches, |obj|{
            let patch: &Patch = obj;
            apply_patch(&mut jwks, *patch);
        });
        *borrow_global_mut<PatchedJWKs>(@depay_framework) = PatchedJWKs { jwks };
    }
    ...
    #[test_only]
    fun initialize_for_test(depay_framework: &signer) {
        create_account_for_test(@depay_framework);
        reconfiguration::initialize_for_test(depay_framework);
        initialize(depay_framework);
    }

    #[test(fx = @depay_framework)]
    fun test_observed_jwks_operations(fx: &signer) acquires ObservedJWKs, PatchedJWKs, Patches {
        ...
        assert!(expected == borrow_global<ObservedJWKs>(@depay_framework).jwks, 1);
        ...
        assert!(expected == borrow_global<ObservedJWKs>(@depay_framework).jwks, 2);
        ...
        assert!(expected == borrow_global<ObservedJWKs>(@depay_framework).jwks, 3);

        remove_issuer_from_observed_jwks(fx, b"alice");
        let expected = AllProvidersJWKs { entries: vector[bob_jwks_v1] };
        assert!(expected == borrow_global<ObservedJWKs>(@depay_framework).jwks, 4);
    }
    ...
    fun test_patched_jwks(depay_framework: signer) acquires ObservedJWKs, PatchedJWKs, Patches {
        ...
        upsert_into_observed_jwks(&depay_framework, vector [
            ProviderJWKs {
                issuer: b"alice",
                version: 111,
                jwks: vector[jwk_0, jwk_1],
        ...
        set_patches(&depay_framework, vector[

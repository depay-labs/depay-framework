...
module depay_framework::coin {
    ...
    use depay_framework::aggregator::Aggregator;
    use depay_framework::event::{Self, EventHandle};
    use depay_framework::guid;
    use depay_framework::optional_aggregator::{Self, OptionalAggregator};
    use depay_framework::system_addresses;

    use depay_framework::fungible_asset::{Self, FungibleAsset, Metadata, MintRef, TransferRef, BurnRef};
    use depay_framework::object::{Self, Object, object_address};
    use depay_framework::primary_fungible_store;
    use depay_std::type_info::{Self, TypeInfo, type_name};
    use depay_framework::create_signer;

    friend depay_framework::depay_coin;
    ...
    const EDPT_PAIRING_IS_NOT_ENABLED: u64 = 28;
    ...
    public fun paired_metadata<CoinType>(): Option<Object<Metadata>> acquires CoinConversionMap {
        if (exists<CoinConversionMap>(@depay_framework) && features::coin_to_fungible_asset_migration_feature_enabled(
        )) {
            let map = &borrow_global<CoinConversionMap>(@depay_framework).coin_to_fungible_asset_map;
    ...
    public entry fun create_coin_conversion_map(depay_framework: &signer) {
        system_addresses::assert_depay_framework(depay_framework);
        if (!exists<CoinConversionMap>(@depay_framework)) {
            move_to(depay_framework, CoinConversionMap {
                coin_to_fungible_asset_map: table::new(),
            })
        };
    }

    /// Create DPT pairing by passing `DePayCoin`.
    ...
    inline fun is_dpt<CoinType>(): bool {
        type_info::type_name<CoinType>() == string::utf8(b"0x1::depay_coin::DePayCoin")
    }

    inline fun create_and_return_paired_metadata_if_not_exist<CoinType>(allow_dpt_creation: bool): Object<Metadata> {
        assert!(
            features::coin_to_fungible_asset_migration_feature_enabled(),
            error::invalid_state(EMIGRATION_FRAMEWORK_NOT_ENABLED)
        );
        assert!(exists<CoinConversionMap>(@depay_framework), error::not_found(ECOIN_CONVERSION_MAP_NOT_FOUND));
        let map = borrow_global_mut<CoinConversionMap>(@depay_framework);
        let type = type_info::type_of<CoinType>();
        if (!table::contains(&map.coin_to_fungible_asset_map, type)) {
            let is_dpt = is_dpt<CoinType>();
            assert!(!is_dpt || allow_dpt_creation, error::invalid_state(EDPT_PAIRING_IS_NOT_ENABLED));
            let metadata_object_cref =
                if (is_dpt) {
                    object::create_sticky_object_at_address(@depay_framework, @depay_fungible_asset)
                } else {
                    object::create_named_object(
                        &create_signer::create_signer(@depay_fungible_asset),
    ...
    public fun allow_supply_upgrades(_depay_framework: &signer, _allowed: bool) {
    ...
    inline fun migrated_primary_fungible_store_exists(
        ...
        fungible_asset::store_exists(primary_store_address) && (
            // migration flag is needed, until we start defaulting new accounts to DPT PFS
            features::new_accounts_default_to_fa_dpt_store_enabled() || exists<MigrationFlag>(primary_store_address)
        )
    }
    ...
    fun mint_internal<CoinType>(amount: u64): Coin<CoinType> acquires CoinInfo {
        ...
        if (option::is_some(maybe_supply)) {
            ...
            spec {
                ...
                use depay_framework::aggregator;
    ...
    #[test(source = @0x2, framework = @depay_framework)]
    ...
    #[test(account = @0x1)]
    #[expected_failure(abort_code = 0x50003, location = depay_framework::fungible_asset)]
    ...
    #[test(framework = @depay_framework, other = @0x123)]
    ...
    #[test(framework = @depay_framework)]
    ...
    #[test(framework = @depay_framework)]
    #[expected_failure(abort_code = 0x20001, location = depay_framework::aggregator)]
    ...
    #[test(account = @depay_framework, aaron = @0xcafe)]
    ...
    #[test(account = @depay_framework, aaron = @0xaa10, bob = @0xb0b)]
    ...
    #[test(account = @depay_framework, aaron = @0xaa10)]

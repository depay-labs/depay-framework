...
module depay_framework::timestamp {
    ...
    /// Marks that time has started. This can only be called from genesis and with the depay_ framework account.
    public(friend) fun set_time_has_started(depay_framework: &signer) {
        system_addresses::assert_depay_framework(depay_framework);
        let timer = CurrentTimeMicroseconds { microseconds: 0 };
        move_to(depay_framework, timer);
    }
    ...
    public fun update_global_time(
        ...
        // Can only be invoked by DePayVM signer.
        system_addresses::assert_vm(account);

        let global_timer = borrow_global_mut<CurrentTimeMicroseconds>(@depay_framework);
    ...
    public fun set_time_has_started_for_testing(account: &signer) {
        if (!exists<CurrentTimeMicroseconds>(@depay_framework)) {
    ...
    public fun now_microseconds(): u64 acquires CurrentTimeMicroseconds {
        borrow_global<CurrentTimeMicroseconds>(@depay_framework).microseconds
    }

spec depay_framework::aggregator_factory {
    use depay_framework::aggregator;
    /// <high-level-req>
    /// No.: 1
    /// Requirement: During the module's initialization, it guarantees that the DePay framework is the caller and that the
    /// AggregatorFactory resource will move under the DePay framework account.
    /// Criticality: High
    ...
    /// resource transfers to the DePay framework account.
    ...
    /// under the DePay account.
    /// Criticality: High
    /// Implementation: The create_aggregator_internal function asserts that AggregatorFactory exists for the DePay
    /// account.
    /// Enforcement: Formally verified via [high-level-req-2](CreateAggregatorInternalAbortsIf).
    ///
    /// No.: 3
    /// Requirement: Only the DePay framework address may create an aggregator instance currently.
    /// Criticality: Low
    /// Implementation: The create_aggregator function ensures that the address calling it is the DePay framework
    /// address.
    ...
    spec initialize_aggregator_factory(depay_framework: &signer) {
        use std::signer;
    ...
    spec schema CreateAggregatorInternalAbortsIf {
        aborts_if !exists<AggregatorFactory>(@depay_framework);
    }

spec depay_framework::staking_contract {
    /// <high-level-req>
    /// No.: 1
    ...
    contract_creation_seed: vector<u8>,
        ...
        include WithdrawAbortsIf<DePayCoin> { account: staker };
    ...
    spec add_stake(staker: &signer, operator: address, amount: u64) {
        ...
        include WithdrawAbortsIf<DePayCoin> { account: staker };
        let balance = global<coin::CoinStore<DePayCoin>>(staker_address).coin.value;
        let post post_coin = global<coin::CoinStore<DePayCoin>>(staker_address).coin.value;
    ...
    spec schema PreconditionsInCreateContract {
        requires exists<stake::ValidatorPerformance>(@depay_framework);
        requires exists<stake::ValidatorSet>(@depay_framework);
        requires exists<staking_config::StakingRewardsConfig>(
            @depay_framework
        ) || !std::features::spec_periodical_reward_rate_decrease_enabled();
        requires exists<depay_framework::timestamp::CurrentTimeMicroseconds>(@depay_framework);
    ...
    spec schema CreateStakePoolAbortsIf {
        ...
        let allowed = global<stake::AllowedValidators>(@depay_framework);
        aborts_if exists<stake::AllowedValidators>(@depay_framework) && !contains(allowed.accounts, resource_addr);

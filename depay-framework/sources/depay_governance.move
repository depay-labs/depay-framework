...
module depay_framework::depay_governance {
    use std::error;
    use std::option;
    use std::signer;
    ...
    use depay_std::simple_map::{Self, SimpleMap};
    use depay_std::smart_table::{Self, SmartTable};
    use depay_std::table::{Self, Table};

    use depay_framework::account::{Self, SignerCapability, create_signer_with_capability};
    use depay_framework::coin;
    use depay_framework::event::{Self, EventHandle};
    use depay_framework::governance_proposal::{Self, GovernanceProposal};
    ...
    use depay_framework::randomness_config;
    use depay_framework::reconfiguration_with_dkg;
    use depay_framework::timestamp;
    use depay_framework::voting;
    ...
    public fun store_signer_cap(
        ...
        if (!exists<GovernanceResponsbility>(@depay_framework)) {
            move_to(
                depay_framework,
        ...
        let signer_caps = &mut borrow_global_mut<GovernanceResponsbility>(@depay_framework).signer_caps;
    ...
    fun initialize(
        ...
        voting::register<GovernanceProposal>(depay_framework);
        move_to(depay_framework, GovernanceConfig {
            voting_duration_secs,
            min_voting_threshold,
            required_proposer_stake,
        });
        move_to(depay_framework, GovernanceEvents {
            create_proposal_events: account::new_event_handle<CreateProposalEvent>(depay_framework),
            update_config_events: account::new_event_handle<UpdateConfigEvent>(depay_framework),
            vote_events: account::new_event_handle<VoteEvent>(depay_framework),
        });
        move_to(depay_framework, VotingRecords {
            votes: table::new(),
        });
        move_to(depay_framework, ApprovedExecutionHashes {
    ...
    public fun update_governance_config(
        ...
        let governance_config = borrow_global_mut<GovernanceConfig>(@depay_framework);
        ...
        if (std::features::module_event_migration_enabled()) {
            ...
            let events = borrow_global_mut<GovernanceEvents>(@depay_framework);
    ...
    public fun initialize_partial_voting(
        ...
        move_to(depay_framework, VotingRecordsV2 {
    ...
    public fun get_voting_duration_secs(): u64 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@depay_framework).voting_duration_secs
    }

    #[view]
    public fun get_min_voting_threshold(): u128 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@depay_framework).min_voting_threshold
    }

    #[view]
    public fun get_required_proposer_stake(): u64 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@depay_framework).required_proposer_stake
    }
    ...
    public fun has_entirely_voted(stake_pool: address, proposal_id: u64): bool acquires VotingRecords {
        ...
        let voting_records = borrow_global<VotingRecords>(@depay_framework);
    ...
    public fun get_remaining_voting_power(
        ...
        if (features::partial_governance_voting_enabled()) {
            let voting_records_v2 = borrow_global<VotingRecordsV2>(@depay_framework);
    ...
    public fun create_proposal_v2_impl(
        ...
        let governance_config = borrow_global<GovernanceConfig>(@depay_framework);
        ...
        let total_voting_token_supply = coin::supply<DePayCoin>();
    ...
    fun vote_internal(
        ...
        if (features::partial_governance_voting_enabled()) {
            let voting_records_v2 = borrow_global_mut<VotingRecordsV2>(@depay_framework);
            ...
            *used_voting_power = *used_voting_power + voting_power;
            ...
            let voting_records = borrow_global_mut<VotingRecords>(@depay_framework);
            assert!(
                !table::contains(&voting_records.votes, record_key),
        ...
        let proposal_state = voting::get_proposal_state<GovernanceProposal>(@depay_framework, proposal_id);
    ...
    public fun add_approved_script_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        let approved_hashes = borrow_global_mut<ApprovedExecutionHashes>(@depay_framework);
        ...
        let execution_hash = voting::get_execution_hash<GovernanceProposal>(@depay_framework, proposal_id);
    ...
    public fun resolve(
        ...
        voting::resolve<GovernanceProposal>(@depay_framework, proposal_id);
    ...
    public fun resolve_multi_step_proposal(
        ...
        voting::resolve_proposal_v2<GovernanceProposal>(@depay_framework, proposal_id, next_execution_hash);
    ...
    public fun remove_approved_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        assert!(
            voting::is_resolved<GovernanceProposal>(@depay_framework, proposal_id),
            error::invalid_argument(EPROPOSAL_NOT_RESOLVED_YET),
        );

        let approved_hashes = &mut borrow_global_mut<ApprovedExecutionHashes>(@depay_framework).hashes;
    ...
    public entry fun reconfigure(depay_framework: &signer) {
        system_addresses::assert_depay_framework(depay_framework);
        if (consensus_config::validator_txn_enabled() && randomness_config::enabled()) {
            reconfiguration_with_dkg::try_start();
        } else {
            reconfiguration_with_dkg::finish(depay_framework);
        }
    }
    ...
    public entry fun force_end_epoch(depay_framework: &signer) {
        system_addresses::assert_depay_framework(depay_framework);
        reconfiguration_with_dkg::finish(depay_framework);
    }
    ...
    public entry fun force_end_epoch_test_only(depay_framework: &signer) acquires GovernanceResponsbility {
        let core_signer = get_signer_testnet_only(depay_framework, @0x1);
        system_addresses::assert_depay_framework(&core_signer);
        reconfiguration_with_dkg::finish(&core_signer);
    }

    /// Update feature flags and also trigger reconfiguration.
    public fun toggle_features(depay_framework: &signer, enable: vector<u64>, disable: vector<u64>) {
        system_addresses::assert_depay_framework(depay_framework);
        features::change_feature_flags_for_next_epoch(depay_framework, enable, disable);
        reconfigure(depay_framework);
    }
    ...
    public fun get_signer_testnet_only(
        ...
        assert!(depay_coin::has_mint_capability(core_resources), error::unauthenticated(EUNAUTHORIZED));
        get_signer(signer_address)
    }
    ...
    fun get_signer(signer_address: address): signer acquires GovernanceResponsbility {
        let governance_responsibility = borrow_global<GovernanceResponsbility>(@depay_framework);
    ...
    fun assert_voting_initialization() {
        if (features::partial_governance_voting_enabled()) {
            assert!(exists<VotingRecordsV2>(@depay_framework), error::invalid_state(EPARTIAL_VOTING_NOT_INITIALIZED));
        };
    }
    ...
    public entry fun test_voting_generic(
        ...
        setup_voting(&depay_framework, &proposer, &yes_voter, &no_voter);
        ...
        test_resolving_proposal_generic(depay_framework, use_generic_resolve_function, execution_hash);
    }
    ...
    public entry fun test_resolving_proposal_generic(
        ...
        let proposal_state = voting::get_proposal_state<GovernanceProposal>(signer::address_of(&depay_framework), 0);
        assert!(proposal_state == PROPOSAL_STATE_SUCCEEDED, proposal_state);

        // Add approved script hash.
        add_approved_script_hash(0);
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@depay_framework).hashes;
        assert!(*simple_map::borrow(&approved_hashes, &0) == execution_hash, 0);

        // Resolve the proposal.
        let account = resolve_proposal_for_test(0, @depay_framework, use_generic_resolve_function, true);
        assert!(signer::address_of(&account) == @depay_framework, 1);
        assert!(voting::is_resolved<GovernanceProposal>(@depay_framework, 0), 2);
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@depay_framework).hashes;
        assert!(!simple_map::contains_key(&approved_hashes, &0), 3);
    }

    #[test(depay_framework = @depay_framework, proposer = @0x123, yes_voter = @0x234, no_voter = @345)]
    public entry fun test_voting(
        ...
        test_voting_generic(depay_framework, proposer, yes_voter, no_voter, false, false);
    }
    ...
    public entry fun test_voting_multi_step(
        ...
        test_voting_generic(depay_framework, proposer, yes_voter, no_voter, true, true);
    }

    #[test(depay_framework = @depay_framework, proposer = @0x123, yes_voter = @0x234, no_voter = @345)]
    #[expected_failure(abort_code = 0x5000a, location = depay_framework::voting)]
    public entry fun test_voting_multi_step_cannot_use_single_step_resolve(
        ...
        test_voting_generic(depay_framework, proposer, yes_voter, no_voter, true, false);
    }
    ...
    public entry fun test_voting_single_step_can_use_generic_resolve_function(
        ...
        test_voting_generic(depay_framework, proposer, yes_voter, no_voter, false, true);
    }
    ...
    public entry fun test_can_remove_approved_hash_if_executed_directly_via_voting_generic(
        ...
        if (multi_step) {
            ...
            vector::push_back(&mut execution_hash, 1);
            voting::resolve_proposal_v2<GovernanceProposal>(@depay_framework, 0, next_execution_hash);
            assert!(voting::is_resolved<GovernanceProposal>(@depay_framework, 0), 0);
            if (vector::length(&next_execution_hash) == 0) {
            ...
            voting::resolve<GovernanceProposal>(@depay_framework, 0);
            assert!(voting::is_resolved<GovernanceProposal>(@depay_framework, 0), 0);
            remove_approved_hash(0);
        };
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@depay_framework).hashes;
    ...
    #[test(depay_framework = @depay_framework, proposer = @0x123, voter_1 = @0x234, voter_2 = @345)]
    #[expected_failure(abort_code = 0x10004, location = depay_framework::voting)]
    public entry fun test_cannot_double_vote(
        ...
        setup_voting(&depay_framework, &proposer, &voter_1, &voter_2);
    ...
    public entry fun test_stake_pool_can_vote_on_partial_voting_proposal_many_times(
        ...
        setup_partial_voting(&depay_framework, &proposer, &voter_1, &voter_2);
        let execution_hash = vector::empty<u8>();
        vector::push_back(&mut execution_hash, 1);
        ...
        test_resolving_proposal_generic(depay_framework, true, execution_hash);
    }
    ...
    public entry fun test_batch_vote(
        ...
        features::change_feature_flags_for_testing(&depay_framework, vector[features::get_coin_to_fungible_asset_migration_feature()], vector[]);
        setup_partial_voting(&depay_framework, &proposer, &voter_1, &voter_2);
        let execution_hash = vector::empty<u8>();
        vector::push_back(&mut execution_hash, 1);
        let voter_1_addr = signer::address_of(&voter_1);
    ...
    public entry fun test_stake_pool_can_vote_before_and_after_partial_governance_voting_enabled(
        ...
        initialize_partial_voting(&depay_framework);
        features::change_feature_flags_for_testing(&depay_framework, vector[features::get_partial_governance_voting()], vector[]);

        coin::register<DePayCoin>(&voter_1);
        coin::register<DePayCoin>(&voter_2);
    ...
    public entry fun test_no_remaining_voting_power_about_proposal_expiration_time(
        ...
        setup_voting_with_initialized_stake(&depay_framework, &proposer, &voter_1, &voter_2);
        let execution_hash = vector::empty<u8>();
        vector::push_back(&mut execution_hash, 1);
        let proposer_addr = signer::address_of(&proposer);
        let voter_1_addr = signer::address_of(&voter_1);
    ...
    public fun setup_voting(
        ...
        staking_config::initialize_for_test(depay_framework, 0, 1000, 2000, true, 0, 1, 100);
        initialize(depay_framework, 10, 100, 1000);
        store_signer_cap(
            depay_framework,
            @depay_framework,
            account::create_test_signer_cap(@depay_framework),
        );
        ...
        stake::create_validator_set(depay_framework, active_validators, pks);
        ...
        coin::destroy_mint_cap<DePayCoin>(mint_cap);
    ...
    public fun setup_voting_with_initialized_stake(
        ...
        // Initialize the governance.
        stake::initialize_for_test_custom(depay_framework, 0, 1000, 2000, true, 0, 1, 1000);
        initialize(depay_framework, 10, 100, 1000);
        store_signer_cap(
            depay_framework,
            @depay_framework,
    ...
    public entry fun test_update_governance_config(
        ...
        account::create_account_for_test(signer::address_of(&depay_framework));
        initialize(&depay_framework, 1, 2, 3);
        update_governance_config(&depay_framework, 10, 20, 30);

        let config = borrow_global<GovernanceConfig>(@depay_framework);
    ...
    #[test(account = @0x123)]
    #[expected_failure(abort_code = 0x50003, location = depay_framework::system_addresses)]
    ...
    public fun initialize_for_test(
        ...
        initialize(depay_framework, min_voting_threshold, required_proposer_stake, voting_duration_secs);
    }

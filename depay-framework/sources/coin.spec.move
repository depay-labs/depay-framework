spec depay_framework::coin {
    ...
    /// Can only be updated by `@depay_framework`.
    spec allow_supply_upgrades(_depay_framework: &signer, _allowed: bool) {
        aborts_if true;
    }
    ...
    spec fun spec_paired_metadata<CoinType>(): Option<Object<Metadata>> {
        if (exists<CoinConversionMap>(@depay_framework)) {
            let map = global<CoinConversionMap>(@depay_framework).coin_to_fungible_asset_map;
    ...
    /// The creator of `CoinType` must be `@depay_framework`.
    ...
    // `account` must be `@depay_framework`.
    ...
    monitor_supply: bool,
        ...
        aborts_if monitor_supply && !exists<aggregator_factory::AggregatorFactory>(@depay_framework);
    ...
    parallelizable: bool,
        ...
        aborts_if monitor_supply && parallelizable
            && !exists<aggregator_factory::AggregatorFactory>(@depay_framework);

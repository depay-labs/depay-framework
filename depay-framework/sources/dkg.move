...
module depay_framework::dkg {
    use std::error;
    use std::option;
    use std::option::Option;
    use depay_framework::event::emit;
    use depay_framework::randomness_config::RandomnessConfig;
    ...
    public(friend) fun start(
        ...
        let dkg_state = borrow_global_mut<DKGState>(@depay_framework);
    ...
    public fun incomplete_session(): Option<DKGSessionState> acquires DKGState {
        if (exists<DKGState>(@depay_framework)) {
            borrow_global<DKGState>(@depay_framework).in_progress
        } else {
            option::none()
        }
    }

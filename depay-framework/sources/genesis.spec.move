spec depay_framework::genesis {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: All the core resources and modules should be created during genesis and owned by the DePay framework
    /// account.
    ...
    /// formally verified by a post condition to ensure that all the critical resources are owned by the DePay framework.
    /// Enforcement: Formally verified via [high-level-req-1](initialize).
    ///
    /// No.: 2
    /// Requirement: Addresses ranging from 0x0 - 0xa should be reserved for the framework and part of depay_ governance.
    /// Criticality: Critical
    ...
    ///
    /// No.: 3
    /// Requirement: The DePay coin should be initialized during genesis and only the DePay framework account should own
    /// the mint and burn capabilities for the DPT token.
    /// Criticality: Critical
    /// Implementation: Both mint and burn capabilities are wrapped inside the stake::DePayCoinCapabilities and
    /// transaction_fee::DePayCoinCapabilities resources which are stored under the depay_ framework account.
    /// Enforcement: Formally verified via [high-level-req-3](initialize_depay_coin).
    ///
    /// No.: 4
    ...
    spec initialize {
        ...
        // property 2: Addresses ranging from 0x0 - 0xa should be reserved for the framework and part of depay_ governance.
        ...
        // property 1: All the core resources and modules should be created during genesis and owned by the DePay framework account.
        /// [high-level-req-1]
        ensures exists<depay_governance::GovernanceResponsbility>(@depay_framework);
        ensures exists<consensus_config::ConsensusConfig>(@depay_framework);
        ensures exists<execution_config::ExecutionConfig>(@depay_framework);
        ensures exists<version::Version>(@depay_framework);
        ensures exists<stake::ValidatorSet>(@depay_framework);
        ensures exists<stake::ValidatorPerformance>(@depay_framework);
        ensures exists<storage_gas::StorageGasConfig>(@depay_framework);
        ensures exists<storage_gas::StorageGas>(@depay_framework);
        ensures exists<gas_schedule::GasScheduleV2>(@depay_framework);
        ensures exists<aggregator_factory::AggregatorFactory>(@depay_framework);
        ensures exists<coin::SupplyConfig>(@depay_framework);
        ensures exists<chain_id::ChainId>(@depay_framework);
        ensures exists<reconfiguration::Configuration>(@depay_framework);
        ensures exists<block::BlockResource>(@depay_framework);
        ensures exists<state_storage::StateStorageUsage>(@depay_framework);
        ensures exists<timestamp::CurrentTimeMicroseconds>(@depay_framework);
        ensures exists<account::Account>(@depay_framework);
        ensures exists<version::SetVersionCapability>(@depay_framework);
        ensures exists<staking_config::StakingConfig>(@depay_framework);
    }

    spec initialize_depay_coin {
        // property 3: The DePay coin should be initialized during genesis and only the DePay framework account should
        // own the mint and burn capabilities for the DPT token.
        /// [high-level-req-3]
        requires !exists<stake::DePayCoinCapabilities>(@depay_framework);
        ensures exists<stake::DePayCoinCapabilities>(@depay_framework);
        requires exists<transaction_fee::DePayCoinCapabilities>(@depay_framework);
        ensures exists<transaction_fee::DePayCoinCapabilities>(@depay_framework);
    }
    ...
    spec set_genesis_end {
        ...
        /// [high-level-req-4]
        requires len(global<stake::ValidatorSet>(@depay_framework).active_validators) >= 1;
        // property 5: The end of genesis should be marked on chain.
        /// [high-level-req-5]
        let addr = std::signer::address_of(depay_framework);
        aborts_if addr != @depay_framework;
        aborts_if exists<chain_status::GenesisEndMarker>(@depay_framework);
        ensures global<chain_status::GenesisEndMarker>(@depay_framework) == chain_status::GenesisEndMarker {};
    }

    spec schema InitalizeRequires {
        execution_config: vector<u8>;
        requires !exists<account::Account>(@depay_framework);
    ...
    spec schema CompareTimeRequires {
        let staking_rewards_config = global<staking_config::StakingRewardsConfig>(@depay_framework);

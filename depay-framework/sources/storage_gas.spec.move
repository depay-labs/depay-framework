spec depay_framework::storage_gas {
    // -----------------
    // Struct invariants
    // -----------------
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGasConfig>(@depay_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGas>(@depay_framework);
    }
    ...
    spec set_config(depay_framework: &signer, config: StorageGasConfig) {
    ...
    spec initialize(depay_framework: &signer) {
        ...
        aborts_if exists<StorageGasConfig>(@depay_framework);
        aborts_if exists<StorageGas>(@depay_framework);

        /// [high-level-req-1]
        ensures exists<StorageGasConfig>(@depay_framework);
        ensures exists<StorageGas>(@depay_framework);
    }
    ...
    spec on_reconfig {
        ...
        aborts_if !exists<StorageGasConfig>(@depay_framework);
        aborts_if !exists<StorageGas>(@depay_framework);
        aborts_if !exists<state_storage::StateStorageUsage>(@depay_framework);
    }

...
module depay_framework::randomness_config {
    use std::string;
    use depay_std::copyable_any;
    use depay_std::copyable_any::Any;
    use depay_std::fixed_point64::FixedPoint64;
    ...
    public fun initialize(framework: &signer, config: RandomnessConfig) {
        ...
        if (!exists<RandomnessConfig>(@depay_framework)) {
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires RandomnessConfig {
        ...
        if (config_buffer::does_exist<RandomnessConfig>()) {
            ...
            if (exists<RandomnessConfig>(@depay_framework)) {
                *borrow_global_mut<RandomnessConfig>(@depay_framework) = new_config;
            } else {
    ...
    public fun enabled(): bool acquires RandomnessConfig {
        if (exists<RandomnessConfig>(@depay_framework)) {
            let config = borrow_global<RandomnessConfig>(@depay_framework);
    ...
    public fun current(): RandomnessConfig acquires RandomnessConfig {
        if (exists<RandomnessConfig>(@depay_framework)) {
            *borrow_global<RandomnessConfig>(@depay_framework)
        } else {
            new_off()
        }
    }

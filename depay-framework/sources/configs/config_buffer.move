...
module depay_framework::config_buffer {
    use std::string::String;
    use depay_std::any;
    use depay_std::any::Any;
    use depay_std::simple_map;
    use depay_std::simple_map::SimpleMap;
    ...
    friend depay_framework::jwks;
    friend depay_framework::jwk_consensus_config;
    friend depay_framework::keyless_account;
    friend depay_framework::randomness_api_v0_config;
    friend depay_framework::randomness_config;
    friend depay_framework::randomness_config_seqnum;
    ...
    public fun initialize(depay_framework: &signer) {
        system_addresses::assert_depay_framework(depay_framework);
        if (!exists<PendingConfigs>(@depay_framework)) {
            move_to(depay_framework, PendingConfigs {
    ...
    public fun does_exist<T: store>(): bool acquires PendingConfigs {
        if (exists<PendingConfigs>(@depay_framework)) {
            let config = borrow_global<PendingConfigs>(@depay_framework);
    ...
    public(friend) fun upsert<T: drop + store>(config: T) acquires PendingConfigs {
        let configs = borrow_global_mut<PendingConfigs>(@depay_framework);

spec depay_framework::state_storage {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: Given the blockchain is in an operating state, the resources for tracking state storage usage and gas
    /// parameters must exist for the DePay framework address.
    /// Criticality: Critical
    /// Implementation: The initialize function ensures only the DePay framework address can call it.
    ...
    /// storage usage will be moved under the DePay framework account with default initial values.
    /// Criticality: Medium
    /// Implementation: The resource for tracking state storage usage may only be initialized with specific values and
    /// published under the depay_framework account.
    ...
    /// usage will be moved under the DePay framework account with default initial values.
    /// Criticality: Medium
    /// Implementation: The resource for tracking state storage usage may only be initialized with specific values and
    /// published under the depay_framework account.
    ...
    spec module {
        ...
        /// [high-level-req-1]
        /// [high-level-req-5.3]
        invariant [suspendable] chain_status::is_operating() ==> exists<StateStorageUsage>(@depay_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<GasParameter>(@depay_framework);
    }
    ...
    spec initialize(depay_framework: &signer) {
        ...
        /// [high-level-req-3]
        aborts_if exists<StateStorageUsage>(@depay_framework);
        ensures exists<StateStorageUsage>(@depay_framework);
        let post state_usage = global<StateStorageUsage>(@depay_framework);
        /// [high-level-req-2]
    ...
    spec on_new_block(epoch: u64) {
        ...
        aborts_if false;
        ensures epoch == global<StateStorageUsage>(@depay_framework).epoch;
    }

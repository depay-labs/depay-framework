spec depay_framework::transaction_validation {
    /// <high-level-req>
    /// No.: 1
    ...
    /// Ensure caller is `depay_framework`.
    ...
    spec schema PrologueCommonAbortsIf {
        use std::bcs;
        use depay_framework::timestamp::{CurrentTimeMicroseconds};
        use depay_framework::chain_id::{ChainId};
        use depay_framework::account::{Account};
        use depay_framework::coin::{CoinStore};
        sender: signer;
        gas_payer: address;
        ...
        aborts_if !exists<CurrentTimeMicroseconds>(@depay_framework);
        aborts_if !(timestamp::now_seconds() < txn_expiration_time);

        aborts_if !exists<ChainId>(@depay_framework);
        ...
        aborts_if !exists<CoinStore<DePayCoin>>(gas_payer);
        // property 1: The sender of a transaction should have sufficient coin balance to pay the transaction fee.
        /// [high-level-req-1]
        aborts_if !(global<CoinStore<DePayCoin>>(gas_payer).coin.value >= max_transaction_fee);
    }
    ...
    /// `DePayCoinCapabilities` and `CoinInfo` should exists.
    ...
    /// Abort according to the conditions.
    /// `DePayCoinCapabilities` and `CoinInfo` should exist.
    /// Skip transaction_fee::burn_fee verification.
    ...
    spec schema EpilogueGasPayerAbortsIf {
        ...
        use depay_framework::coin::{CoinStore, CoinInfo};
        use depay_framework::optional_aggregator;
        use depay_framework::transaction_fee::{DePayCoinCapabilities, DePayCoinMintCapability};
        ...
        // TODO(fa_migration)
        // let pre_balance = global<coin::CoinStore<DePayCoin>>(gas_payer).coin.value;
        // let post balance = global<coin::CoinStore<DePayCoin>>(gas_payer).coin.value;
        ...
        let dpt_addr = type_info::type_of<DePayCoin>().account_address;
        let maybe_dpt_supply = global<CoinInfo<DePayCoin>>(dpt_addr).supply;
        let total_supply_enabled = option::spec_is_some(maybe_dpt_supply);
        let dpt_supply = option::spec_borrow(maybe_dpt_supply);
        let dpt_supply_value = optional_aggregator::optional_aggregator_value(dpt_supply);
        let post post_maybe_dpt_supply = global<CoinInfo<DePayCoin>>(dpt_addr).supply;
        let post post_dpt_supply = option::spec_borrow(post_maybe_dpt_supply);
        let post post_dpt_supply_value = optional_aggregator::optional_aggregator_value(post_dpt_supply);

        aborts_if amount_to_burn > 0 && !exists<DePayCoinCapabilities>(@depay_framework);
        aborts_if amount_to_burn > 0 && !exists<CoinInfo<DePayCoin>>(dpt_addr);
        aborts_if amount_to_burn > 0 && total_supply_enabled && dpt_supply_value < amount_to_burn;
        ensures total_supply_enabled ==> dpt_supply_value - amount_to_burn == post_dpt_supply_value;

        // Check minting.
        let amount_to_mint = storage_fee_refunded - transaction_fee_amount;
        let total_supply = coin::supply<DePayCoin>;
        let post post_total_supply = coin::supply<DePayCoin>;

        aborts_if amount_to_mint > 0 && !exists<CoinStore<DePayCoin>>(addr);
        aborts_if amount_to_mint > 0 && !exists<DePayCoinMintCapability>(@depay_framework);
        ...
        aborts_if (amount_to_mint != 0) && !exists<coin::CoinInfo<DePayCoin>>(depay_addr);
        include coin::CoinAddAbortsIf<DePayCoin> { amount: amount_to_mint };
    }
}

...
/// [AIP-61](https://github.com/depay_-foundation/AIPs/blob/main/aips/aip-61.md).
module depay_framework::keyless_account {
    ...
    use std::vector;
    use depay_std::crypto_algebra;
    use depay_std::ed25519;
    use depay_framework::chain_status;
    use depay_framework::system_addresses;

    // The `depay_framework::reconfiguration_with_dkg` module needs to be able to call `on_new_epoch`.
    ...
    #[resource_group_member(group = depay_framework::keyless_account::Group)]
    ...
    public(friend) fun on_new_epoch(fx: &signer) acquires Groth16VerificationKey, Configuration {
        ...
        if (config_buffer::does_exist<Groth16VerificationKey>()) {
            ...
            if (exists<Groth16VerificationKey>(@depay_framework)) {
                *borrow_global_mut<Groth16VerificationKey>(@depay_framework) = vk;
            } else {
                move_to(fx, vk);
            }
        };
        ...
        if (config_buffer::does_exist<Configuration>()) {
            ...
            if (exists<Configuration>(@depay_framework)) {
                *borrow_global_mut<Configuration>(@depay_framework) = config;
            } else {
                move_to(fx, config);
            }
        };

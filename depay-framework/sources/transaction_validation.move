module depay_framework::transaction_validation {
    use std::bcs;
    use std::error;
    use std::features;
    use std::signer;
    ...
    public(friend) fun initialize(
        ...
        move_to(depay_framework, TransactionValidation {
    ...
    fun prologue_common(
        ...
        if (!features::transaction_simulation_enhancement_enabled() || !skip_gas_payment(is_simulation, gas_payer)) {
            if (features::operations_default_to_fa_dpt_store_enabled()) {
                assert!(
                    depay_account::is_fungible_balance_at_least(gas_payer, max_transaction_fee),
                    error::invalid_argument(PROLOGUE_ECANT_PAY_GAS_DEPOSIT)
                );
            } else {
                assert!(
                    coin::is_balance_at_least<DePayCoin>(gas_payer, max_transaction_fee),
                    error::invalid_argument(PROLOGUE_ECANT_PAY_GAS_DEPOSIT)
                );
            }
        }
    ...
    // Once the transaction_simulation_enhancement feature is enabled, the DePay VM will invoke this function instead.
    ...
    fun epilogue_gas_payer_extended(
        ...
        if (!features::transaction_simulation_enhancement_enabled() || !skip_gas_payment(is_simulation, gas_payer)) {
            if (features::operations_default_to_fa_dpt_store_enabled()) {
                assert!(
                    depay_account::is_fungible_balance_at_least(gas_payer, transaction_fee_amount),
                    error::out_of_range(PROLOGUE_ECANT_PAY_GAS_DEPOSIT),
                );
            } else {
                assert!(
                    coin::is_balance_at_least<DePayCoin>(gas_payer, transaction_fee_amount),
                    error::out_of_range(PROLOGUE_ECANT_PAY_GAS_DEPOSIT),
                );
            };

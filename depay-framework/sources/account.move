module depay_framework::account {
    use std::bcs;
    use std::error;
    use std::hash;
    ...
    use depay_std::ed25519;
    use depay_std::from_bcs;
    use depay_std::multi_ed25519;
    ...
    #[test_only]
    /// Create signer for testing, independently of an DePay-style `Account`.
    ...
    public(friend) fun initialize(depay_framework: &signer) {
        ...
        move_to(depay_framework, OriginatingAddress {
    ...
    public fun originating_address(auth_key: address): Option<address> acquires OriginatingAddress {
        let address_map_ref = &borrow_global<OriginatingAddress>(@depay_framework).address_map;
    ...
    entry fun set_originating_address(account: &signer) acquires Account, OriginatingAddress {
        ...
        let address_map_ref_mut =
            &mut borrow_global_mut<OriginatingAddress>(@depay_framework).address_map;
    ...
    fun update_auth_key_and_originating_address_table(
        ...
        let address_map = &mut borrow_global_mut<OriginatingAddress>(@depay_framework).address_map;
    ...
    /// `create_account` to attempt to claim an account ahead of the creation of a resource account, if found DePay will
    ...
    fun test_create_signer_for_test() {
        assert!(signer::address_of(&create_signer_for_test(@depay_framework)) == @0x1, 0);
    ...
    #[test(alice = @0xa11ce)]
    #[expected_failure(abort_code = 65537, location = depay_framework::ed25519)]
    ...
    //
    #[test(bob = @0x345, framework = @depay_framework)]
    ...
    #[test(bob = @0x345, charlie = @0x567, framework = @depay_framework)]

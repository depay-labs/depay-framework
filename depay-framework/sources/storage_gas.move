...
module depay_framework::storage_gas {
    ...
    public(friend) fun set_config(depay_framework: &signer, config: StorageGasConfig) acquires StorageGasConfig {
        system_addresses::assert_depay_framework(depay_framework);
        *borrow_global_mut<StorageGasConfig>(@depay_framework) = config;
    }
    ...
    public fun initialize(depay_framework: &signer) {
        system_addresses::assert_depay_framework(depay_framework);
        assert!(
            !exists<StorageGasConfig>(@depay_framework),
        ...
        move_to(depay_framework, StorageGasConfig {
            item_config,
            byte_config,
        });

        assert!(
            !exists<StorageGas>(@depay_framework),
            error::already_exists(ESTORAGE_GAS)
        );
        move_to(depay_framework, StorageGas {
            per_item_read: 300 * k,
            per_item_create: 5 * m,
    ...
    public(friend) fun on_reconfig() acquires StorageGas, StorageGasConfig {
        ...
        let gas_config = borrow_global<StorageGasConfig>(@depay_framework);
        let gas = borrow_global_mut<StorageGas>(@depay_framework);
    ...
    fun test_initialize_and_reconfig(framework: signer) acquires StorageGas, StorageGasConfig {
        ...
        on_reconfig();
        let gas_parameter = borrow_global<StorageGas>(@depay_framework);

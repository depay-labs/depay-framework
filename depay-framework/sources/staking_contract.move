...
module depay_framework::staking_contract {
    use std::bcs;
    use std::error;
    use std::features;
    use std::signer;
    ...
    use depay_framework::account::{Self, SignerCapability};
    ...
    use depay_framework::stake::{Self, OwnerCapability};
    use depay_framework::staking_config;

    const SALT: vector<u8> = b"depay_framework::staking_contract";
    ...
    #[resource_group_member(group = depay_framework::staking_contract::StakingGroupContainer)]
    ...
    public entry fun create_staking_contract(
        ...
        let staked_coins = coin::withdraw<DePayCoin>(staker, amount);
    ...
    fun distribute_internal(
        ...
        while (pool_u64::shareholders_count(distribution_pool) > 0) {
            ...
            depay_account::deposit_coins(recipient, coin::extract(&mut coins, amount_to_distribute));
        ...
        if (coin::value(&coins) > 0) {
            depay_account::deposit_coins(staker, coins);
    ...
    #[test_only]
    const INITIAL_BALANCE: u64 = 100000000000000; // 1M DPT coins with 8 decimals.

    #[test_only]
    const MAXIMUM_STAKE: u64 = 100000000000000000; // 1B DPT coins with 8 decimals.
    ...
    #[test_only]
    public fun setup(depay_framework: &signer, staker: &signer, operator: &signer, initial_balance: u64) {
    ...
    public fun setup_staking_contract(
        ...
        setup(depay_framework, staker, operator, amount);
        let operator_address = signer::address_of(operator);
    ...
    #[test(depay_framework = @0x1, staker = @0x123, operator = @0x234)]
    public entry fun test_end_to_end(
        depay_framework: &signer,
        staker: &signer,
        operator: &signer
    ) acquires Store, BeneficiaryForOperator {
        setup_staking_contract(depay_framework, staker, operator, INITIAL_BALANCE, 10);
        let staker_address = signer::address_of(staker);
        let operator_address = signer::address_of(operator);
        ...
        let operator_balance = coin::balance<DePayCoin>(operator_address);
        ...
        distribute(staker_address, operator_address);
        operator_balance = coin::balance<DePayCoin>(operator_address);
        ...
        let staker_balance = coin::balance<DePayCoin>(staker_address);
    ...
    public entry fun test_operator_cannot_request_same_commission_multiple_times(
        depay_framework: &signer, staker: &signer, operator: &signer) acquires Store, BeneficiaryForOperator {
        setup_staking_contract(depay_framework, staker, operator, INITIAL_BALANCE, 10);
        let staker_address = signer::address_of(staker);
        let operator_address = signer::address_of(operator);
        let pool_address = stake_pool_address(staker_address, operator_address);
    ...
    public entry fun test_staker_cannot_create_staking_contract_with_invalid_commission(
        ...
        setup_staking_contract(depay_framework, staker, operator, INITIAL_BALANCE, 101);
    }
    ...
    public entry fun test_staker_cannot_create_staking_contract_with_less_than_min_stake_required(
        ...
        setup_staking_contract(depay_framework, staker, operator, 50, 100);
    }
    ...
    #[test(depay_framework = @0x1, staker = @0x123, operator_1 = @0x234, operator_2 = @0x345)]
    public entry fun test_staker_can_switch_operator(
        ...
        setup_staking_contract(depay_framework, staker, operator_1, INITIAL_BALANCE, 10);
        ...
        let operator_1_balance = coin::balance<DePayCoin>(operator_1_address);
        ...
        let operator_2_balance = coin::balance<DePayCoin>(operator_2_address);
        new_balance = with_rewards(new_balance);
    ...
    #[test(depay_framework = @0x1, staker = @0x123, operator1 = @0x234, beneficiary = @0x345, operator2 = @0x456)]
    public entry fun test_operator_can_set_beneficiary(
        ...
        setup_staking_contract(depay_framework, staker, operator1, INITIAL_BALANCE, 10);
        let staker_address = signer::address_of(staker);
        ...
        depay_framework::depay_account::create_account(beneficiary_address);
        ...
        let operator_balance = coin::balance<DePayCoin>(operator1_address);
        let beneficiary_balance = coin::balance<DePayCoin>(beneficiary_address);
        ...
        assert!(coin::balance<DePayCoin>(operator2_address) >= expected_commission, 1);

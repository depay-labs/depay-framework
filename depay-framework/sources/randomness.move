...
/// [AIP-41](https://github.com/depay_-foundation/AIPs/blob/main/aips/aip-41.md).
///
...
module depay_framework::randomness {
    use std::hash;
    use std::option;
    ...
    const DST: vector<u8> = b"DEPAY_RANDOMNESS";
    ...
    public fun initialize(framework: &signer) {
        ...
        if (!exists<PerBlockRandomness>(@depay_framework)) {
    ...
    public(friend) fun on_new_block(vm: &signer, epoch: u64, round: u64, seed_for_new_block: Option<vector<u8>>) acquires PerBlockRandomness {
        ...
        if (exists<PerBlockRandomness>(@depay_framework)) {
            let randomness = borrow_global_mut<PerBlockRandomness>(@depay_framework);
    ...
    fun next_32_bytes(): vector<u8> acquires PerBlockRandomness {
        ...
        let input = DST;
        let randomness = borrow_global<PerBlockRandomness>(@depay_framework);

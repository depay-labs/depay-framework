...
module depay_framework::multisig_account {
    use depay_framework::account::{Self, SignerCapability, new_event_handle, create_resource_address};
    ...
    use depay_framework::timestamp::now_seconds;
    ...
    /// account.
    const DOMAIN_SEPARATOR: vector<u8> = b"depay_framework::multisig_account";
    ...
    fun create_multisig_account(owner: &signer): (signer, SignerCapability) {
        ...
        // Register the account to receive DPT as this is not done by default as part of the resource account creation
        // flow.
        if (!coin::is_account_registered<DePayCoin>(address_of(&multisig_signer))) {
            coin::register<DePayCoin>(&multisig_signer);
        };
    ...
    #[test_only]
    use depay_framework::coin::{destroy_mint_cap, destroy_burn_cap};
    ...
    fun setup() {
        ...
        let (burn, mint) = depay_coin::initialize_for_test(framework_signer);

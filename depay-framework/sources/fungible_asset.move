...
module depay_framework::fungible_asset {
    use depay_framework::aggregator_v2::{Self, Aggregator};
    use depay_framework::create_signer;
    use depay_framework::event;
    use depay_framework::function_info::{Self, FunctionInfo};
    use depay_framework::object::{Self, Object, ConstructorRef, DeleteRef, ExtendRef};
    use std::string;
    use std::features;
    ...
    friend depay_framework::primary_fungible_store;
    friend depay_framework::depay_account;

    friend depay_framework::dispatchable_fungible_asset;
    ...
    const EDPT_NOT_DISPATCHABLE: u64 = 31;
    /// Flag for Concurrent Supply not enabled
    ...
    inline fun register_dispatch_function_sanity_check(
        ...
        assert!(
            object::address_from_constructor_ref(constructor_ref) != @depay_fungible_asset,
            error::permission_denied(EDPT_NOT_DISPATCHABLE)
        );
        assert!(
    ...
    fun has_deposit_dispatch_function(metadata: Object<Metadata>): bool acquires DispatchFunctionStore {
        ...
        if(metadata_addr != @depay_fungible_asset && exists<DispatchFunctionStore>(metadata_addr)) {
    ...
    #[test(fx = @depay_framework, creator = @0xcafe)]

spec depay_framework::stake {
    ...
    /// Criticality: Low
    /// Implementation: The total staked value (DePayCoin) of a stake pool is grouped by: active, inactive,
    ...
    spec module {
        ...
        invariant [suspendable] exists<ValidatorSet>(@depay_framework) ==> validator_set_is_valid();
        // After genesis, `DePayCoinCapabilities`, `ValidatorPerformance` and `ValidatorSet` exist.
        invariant [suspendable] chain_status::is_operating() ==> exists<DePayCoinCapabilities>(@depay_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<ValidatorPerformance>(@depay_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<ValidatorSet>(@depay_framework);
    ...
    spec schema ValidatorNotChangeDuringReconfig {
        ensures (reconfiguration_state::spec_is_in_progress() && old(exists<ValidatorSet>(@depay_framework))) ==>
            old(global<ValidatorSet>(@depay_framework)) == global<ValidatorSet>(@depay_framework);
    }
    ...
    spec fun validator_set_is_valid(): bool {
        let validator_set = global<ValidatorSet>(@depay_framework);
    ...
    spec initialize_validator(
        ...
        let allowed = global<AllowedValidators>(@depay_framework);
        aborts_if exists<ValidatorConfig>(addr);
        aborts_if exists<AllowedValidators>(@depay_framework) && !vector::spec_contains(allowed.accounts, addr);
    ...
    spec initialize(depay_framework: &signer) {
        ...
        let depay_addr = signer::address_of(depay_framework);
        aborts_if !system_addresses::is_depay_framework_address(depay_addr);
        aborts_if exists<ValidatorSet>(depay_addr);
        aborts_if exists<ValidatorPerformance>(depay_addr);
        ensures exists<ValidatorSet>(depay_addr);
        ensures global<ValidatorSet>(depay_addr).consensus_scheme == 0;
        ensures exists<ValidatorPerformance>(depay_addr);
    }
    ...
    )
    {
        ...
        aborts_if !exists<StakingConfig>(@depay_framework);
        aborts_if !exists<ValidatorSet>(@depay_framework);
        aborts_if reconfiguration_state::spec_is_in_progress();

        let stake_pool = global<StakePool>(pool_address);
        let validator_set = global<ValidatorSet>(@depay_framework);
        let post p_validator_set = global<ValidatorSet>(@depay_framework);
    ...
    )
    {
        ...
        let bool_find_validator = !option::spec_is_some(spec_find_validator(validator_set.active_validators, pool_address)) &&
                    !option::spec_is_some(spec_find_validator(validator_set.pending_inactive, pool_address)) &&
                        !option::spec_is_some(spec_find_validator(validator_set.pending_active, pool_address));
        aborts_if bool_find_validator && !exists<timestamp::CurrentTimeMicroseconds>(@depay_framework);
        ...
        aborts_if !exists<coin::CoinStore<DePayCoin>>(addr);
        include coin::DepositAbortsIf<DePayCoin>{account_addr: addr};

        let coin_store = global<coin::CoinStore<DePayCoin>>(addr);
        let post p_coin_store = global<coin::CoinStore<DePayCoin>>(addr);
        ensures bool_find_validator && timestamp::now_seconds() > stake_pool.locked_until_secs
                    && exists<account::Account>(addr) && exists<coin::CoinStore<DePayCoin>>(addr) ==>
                        coin_store.coin.value + new_withdraw_amount_1 == p_coin_store.coin.value;
        ensures !(bool_find_validator && exists<timestamp::CurrentTimeMicroseconds>(@depay_framework))
                    && exists<account::Account>(addr) && exists<coin::CoinStore<DePayCoin>>(addr) ==>
    ...
    spec leave_validator_set(
        ...
        let post post_validator_set = global<ValidatorSet>(@depay_framework);
    ...
    spec unlock_with_cap(amount: u64, owner_cap: &OwnerCapability) {
        ...
        let min_amount = depay_std::math64::min(amount,pre_stake_pool.active.value);
    ...
    spec increase_lockup_with_cap(owner_cap: &OwnerCapability) {
        let config = global<staking_config::StakingConfig>(@depay_framework);
        let pool_address = owner_cap.pool_address;
        let pre_stake_pool = global<StakePool>(pool_address);
        let post stake_pool = global<StakePool>(pool_address);
    ...
    spec reactivate_stake_with_cap(owner_cap: &OwnerCapability, amount: u64) {
        ...
        let post stake_pool = global<StakePool>(pool_address);
        modifies global<StakePool>(pool_address);
        let min_amount = depay_std::math64::min(amount, pre_stake_pool.pending_inactive.value);
    ...
    spec on_new_epoch {
        ...
        include depay_framework::depay_coin::ExistsDePayCoin;
    ...
    spec update_performance_statistics {
        ...
        let validator_perf = global<ValidatorPerformance>(@depay_framework);
        let post post_validator_perf = global<ValidatorPerformance>(@depay_framework);
    ...
    spec schema UpdateStakePoolAbortsIf {
        ...
        let depay_addr = type_info::type_of<DePayCoin>().account_address;
    ...
    spec schema DistributeRewardsAbortsIf {
        ...
        let addr = type_info::type_of<DePayCoin>().account_address;
        aborts_if (rewards_amount > 0) && !exists<coin::CoinInfo<DePayCoin>>(addr);
        modifies global<coin::CoinInfo<DePayCoin>>(addr);
        include (rewards_amount > 0) ==> coin::CoinAddAbortsIf<DePayCoin> { amount: amount };
    }
    ...
    spec fun spec_get_reconfig_start_time_secs(): u64 {
        if (exists<reconfiguration_state::State>(@depay_framework)) {
    ...
    spec update_voting_power_increase(increase_amount: u64) {
        ...
        let depay_ = @depay_framework;
        let pre_validator_set = global<ValidatorSet>(depay_);
        let post validator_set = global<ValidatorSet>(depay_);
        let staking_config = global<staking_config::StakingConfig>(depay_);
    ...
    spec configure_allowed_validators(depay_framework: &signer, accounts: vector<address>) {
        let depay_framework_address = signer::address_of(depay_framework);
        aborts_if !system_addresses::is_depay_framework_address(depay_framework_address);
        let post allowed = global<AllowedValidators>(depay_framework_address);
    ...
    spec fun spec_validator_index_upper_bound(): u64 {
        len(global<ValidatorPerformance>(@depay_framework).validators)
    }
    ...
    spec schema ResourceRequirement {
        requires exists<DePayCoinCapabilities>(@depay_framework);
        requires exists<ValidatorPerformance>(@depay_framework);
        requires exists<ValidatorSet>(@depay_framework);
        requires exists<StakingConfig>(@depay_framework);
        requires exists<StakingRewardsConfig>(@depay_framework) || !features::spec_periodical_reward_rate_decrease_enabled();
        requires exists<timestamp::CurrentTimeMicroseconds>(@depay_framework);
    }
    ...
    spec fun spec_get_reward_rate_1(config: StakingConfig): num {
        if (features::spec_periodical_reward_rate_decrease_enabled()) {
            let epoch_rewards_rate = global<staking_config::StakingRewardsConfig>(@depay_framework).rewards_rate;
            if (epoch_rewards_rate.value == 0) {
                0
            } else {
                let denominator_0 = depay_std::fixed_point64::spec_divide_u128(staking_config::MAX_REWARDS_RATE, epoch_rewards_rate);
                let denominator = if (denominator_0 > MAX_U64) {
                    MAX_U64
                } else {
                    denominator_0
                };
                let nominator = depay_std::fixed_point64::spec_multiply_u128(denominator, epoch_rewards_rate);
                nominator
            }
        } else {
                config.rewards_rate

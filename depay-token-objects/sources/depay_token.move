...
module depay_token_objects::depay_token {
    use std::error;
    use std::option::{Self, Option};
    use std::string::String;
    use std::signer;
    use depay_framework::object::{Self, ConstructorRef, Object};
    use depay_token_objects::collection;
    use depay_token_objects::property_map;
    use depay_token_objects::royalty;
    use depay_token_objects::token;
    ...
    #[resource_group_member(group = depay_framework::object::ObjectGroup)]
    /// Storage state for managing the no-code Collection.
    struct DePayCollection has key {
    ...
    public fun create_collection_object(
        ...
        let depay_collection = DePayCollection {
            mutator_ref,
            royalty_mutator_ref,
            mutable_description,
            mutable_uri,
        ...
        move_to(&object_signer, depay_collection);
    ...
    ) acquires DePayCollection, DePayToken {
    ...
    public fun mint_token_object(
        ...
        property_types: vector<String>,
        property_values: vector<vector<u8>>,
    ): Object<DePayToken> acquires DePayCollection, DePayToken {
        ...
        if (freezable_by_creator) {
            let depay_token_addr = object::address_from_constructor_ref(&constructor_ref);
            let depay_token = borrow_global_mut<DePayToken>(depay_token_addr);
            let transfer_ref = object::generate_transfer_ref(&constructor_ref);
            option::fill(&mut depay_token.transfer_ref, transfer_ref);
        };
    ...
    public fun mint_soul_bound_token_object(
        ...
        property_values: vector<vector<u8>>,
        soul_bound_to: address,
    ): Object<DePayToken> acquires DePayCollection {
        let constructor_ref = mint_internal(
            creator,
            collection,
            description,
    ...
    fun mint_internal(
        ...
        property_types: vector<String>,
        property_values: vector<vector<u8>>,
    ): ConstructorRef acquires DePayCollection {
    ...
    inline fun borrow<T: key>(token: &Object<T>): &DePayToken {
        let token_address = object::object_address(token);
        assert!(
            exists<DePayToken>(token_address),
            error::not_found(ETOKEN_DOES_NOT_EXIST),
        );
        borrow_global<DePayToken>(token_address)
    }

    #[view]
    public fun are_properties_mutable<T: key>(token: Object<T>): bool acquires DePayCollection {
        let collection = token::collection_object(token);
        borrow_collection(&collection).mutable_token_properties
    }

    #[view]
    public fun is_burnable<T: key>(token: Object<T>): bool acquires DePayToken {
        option::is_some(&borrow(&token).burn_ref)
    }

    #[view]
    public fun is_freezable_by_creator<T: key>(token: Object<T>): bool acquires DePayCollection {
        are_collection_tokens_freezable(token::collection_object(token))
    }

    #[view]
    public fun is_mutable_description<T: key>(token: Object<T>): bool acquires DePayCollection {
        is_mutable_collection_token_description(token::collection_object(token))
    }

    #[view]
    public fun is_mutable_name<T: key>(token: Object<T>): bool acquires DePayCollection {
        is_mutable_collection_token_name(token::collection_object(token))
    }

    #[view]
    public fun is_mutable_uri<T: key>(token: Object<T>): bool acquires DePayCollection {
        is_mutable_collection_token_uri(token::collection_object(token))
    }

    // Token mutators

    inline fun authorized_borrow<T: key>(token: &Object<T>, creator: &signer): &DePayToken {
        let token_address = object::object_address(token);
        assert!(
            exists<DePayToken>(token_address),
            error::not_found(ETOKEN_DOES_NOT_EXIST),
    ...
    public entry fun burn<T: key>(creator: &signer, token: Object<T>) acquires DePayToken {
        let depay_token = authorized_borrow(&token, creator);
        assert!(
            option::is_some(&depay_token.burn_ref),
            error::permission_denied(ETOKEN_NOT_BURNABLE),
        );
        move depay_token;
        let depay_token = move_from<DePayToken>(object::object_address(&token));
        let DePayToken {
            burn_ref,
            transfer_ref: _,
            mutator_ref: _,
    ...
    public entry fun freeze_transfer<T: key>(creator: &signer, token: Object<T>) acquires DePayCollection, DePayToken {
        let depay_token = authorized_borrow(&token, creator);
        assert!(
            are_collection_tokens_freezable(token::collection_object(token))
                && option::is_some(&depay_token.transfer_ref),
            error::permission_denied(EFIELD_NOT_MUTABLE),
        );
        object::disable_ungated_transfer(option::borrow(&depay_token.transfer_ref));
    }
    ...
    public entry fun unfreeze_transfer<T: key>(
        ...
        object::enable_ungated_transfer(option::borrow(&depay_token.transfer_ref));
    }
    ...
    public entry fun set_description<T: key>(
        ...
        let depay_token = authorized_borrow(&token, creator);
        token::set_description(option::borrow(&depay_token.mutator_ref), description);
    }
    ...
    public entry fun set_name<T: key>(
        ...
        let depay_token = authorized_borrow(&token, creator);
        token::set_name(option::borrow(&depay_token.mutator_ref), name);
    }
    ...
    public entry fun set_uri<T: key>(
        ...
        let depay_token = authorized_borrow(&token, creator);
        token::set_uri(option::borrow(&depay_token.mutator_ref), uri);
    }
    ...
    public entry fun add_property<T: key>(
        ...
        property_map::add(&depay_token.property_mutator_ref, key, type, value);
    }
    ...
    public entry fun add_typed_property<T: key, V: drop>(
        ...
        property_map::add_typed(&depay_token.property_mutator_ref, key, value);
    }
    ...
    public entry fun remove_property<T: key>(
        ...
        property_map::remove(&depay_token.property_mutator_ref, &key);
    }
    ...
    public entry fun update_property<T: key>(
        ...
        property_map::update(&depay_token.property_mutator_ref, &key, type, value);
    }
    ...
    public entry fun update_typed_property<T: key, V: drop>(
        ...
        property_map::update_typed(&depay_token.property_mutator_ref, &key, value);
    }

    // Collection accessors

    inline fun collection_object(creator: &signer, name: &String): Object<DePayCollection> {
        let collection_addr = collection::create_collection_address(&signer::address_of(creator), name);
        object::address_to_object<DePayCollection>(collection_addr)
    }

    inline fun borrow_collection<T: key>(token: &Object<T>): &DePayCollection {
        let collection_address = object::object_address(token);
        assert!(
            exists<DePayCollection>(collection_address),
            error::not_found(ECOLLECTION_DOES_NOT_EXIST),
        );
        borrow_global<DePayCollection>(collection_address)
    }

    public fun is_mutable_collection_description<T: key>(
        collection: Object<T>,
    ): bool acquires DePayCollection {
    ...
    inline fun authorized_borrow_collection<T: key>(collection: &Object<T>, creator: &signer): &DePayCollection {
    ...
    public entry fun set_collection_description<T: key>(
        ...
        let depay_collection = authorized_borrow_collection(&collection, creator);
        assert!(
            depay_collection.mutable_description,
            error::permission_denied(EFIELD_NOT_MUTABLE),
        );
        collection::set_description(option::borrow(&depay_collection.mutator_ref), description);
    }
    ...
    public fun set_collection_royalties<T: key>(
        ...
        let depay_collection = authorized_borrow_collection(&collection, creator);
        assert!(
            option::is_some(&depay_collection.royalty_mutator_ref),
            error::permission_denied(EFIELD_NOT_MUTABLE),
        );
        royalty::update(option::borrow(&depay_collection.royalty_mutator_ref), royalty);
    }
    ...
    public entry fun set_collection_uri<T: key>(
        ...
        collection::set_uri(option::borrow(&depay_collection.mutator_ref), uri);
    }
    ...
    #[test(creator = @0x123)]
    fun test_create_and_transfer(creator: &signer) acquires DePayCollection, DePayToken {
    ...
    fun test_mint_soul_bound(creator: &signer, bob: &signer) acquires DePayCollection {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50003, location = object)]
    fun test_frozen_transfer(creator: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_unfrozen_transfer(creator: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    fun test_noncreator_freeze(creator: &signer, another: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123, another = @0x456)]
    #[expected_failure(abort_code = 0x50003, location = Self)]
    fun test_noncreator_unfreeze(creator: &signer, another: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_set_description(creator: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    fun test_set_immutable_description(creator: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_set_name(creator: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50004, location = Self)]
    fun test_set_immutable_name(creator: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_set_uri(creator: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50004, location = Self)]
    fun test_set_immutable_uri(creator: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_burnable(creator: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
        ...
        assert!(exists<DePayToken>(token_addr), 0);
        burn(creator, token);
        assert!(!exists<DePayToken>(token_addr), 1);
    }

    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50005, location = Self)]
    fun test_not_burnable(creator: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_set_collection_description(creator: &signer) acquires DePayCollection {
        let collection_name = string::utf8(b"collection name");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50004, location = Self)]
    fun test_set_immutable_collection_description(creator: &signer) acquires DePayCollection {
        let collection_name = string::utf8(b"collection name");
    ...
    #[test(creator = @0x123)]
    fun test_set_collection_uri(creator: &signer) acquires DePayCollection {
        let collection_name = string::utf8(b"collection name");
        let collection = create_collection_helper(creator, collection_name, true);
        let value = string::utf8(b"not");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50004, location = Self)]
    fun test_set_immutable_collection_uri(creator: &signer) acquires DePayCollection {
        let collection_name = string::utf8(b"collection name");
        let collection = create_collection_helper(creator, collection_name, false);
    ...
    #[test(creator = @0x123)]
    fun test_property_add(creator: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_property_typed_add(creator: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
        let property_name = string::utf8(b"u8");
        ...
        create_collection_helper(creator, collection_name, true);
        let token = mint_helper(creator, collection_name, token_name);
        add_typed_property<DePayToken, u8>(creator, token, property_name, 0x8);

        assert!(property_map::read_u8(&token, &property_name) == 0x8, 0);
    }

    #[test(creator = @0x123)]
    fun test_property_update(creator: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_property_update_typed(creator: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
        let property_name = string::utf8(b"bool");
        ...
        create_collection_helper(creator, collection_name, true);
        let token = mint_helper(creator, collection_name, token_name);
        update_typed_property<DePayToken, bool>(creator, token, property_name, false);

        assert!(!property_map::read_bool(&token, &property_name), 0);
    }

    #[test(creator = @0x123)]
    fun test_property_remove(creator: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
        let property_name = string::utf8(b"bool");
    ...
    #[test(creator = @0x123)]
    fun test_royalties(creator: &signer) acquires DePayCollection, DePayToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");

...
module depay_token_objects::collection {
    use std::error;
    use std::features;
    use std::option::{Self, Option};
    use std::signer;
    use std::string::{Self, String};
    use depay_framework::aggregator_v2::{Self, Aggregator, AggregatorSnapshot};
    use depay_framework::event;
    use depay_framework::object::{Self, ConstructorRef, ExtendRef, Object};

    use depay_token_objects::royalty::{Self, Royalty};
    #[test_only]
    use std::vector;

    friend depay_token_objects::token;
    ...
    #[expected_failure(abort_code = 0x50003, location = depay_framework::object)]
    ...
    entry fun test_create_and_transfer_as_owner(creator: &signer, trader: &signer, depay_framework: &signer) {
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x1000A, location = depay_token_objects::collection)]
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x20009, location = depay_token_objects::collection)]
